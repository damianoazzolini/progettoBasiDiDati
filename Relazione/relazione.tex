...
\vspace{0.5cm}
\end{center}
La sottoclasse \textit{Paziente} è una specializzazione della entità \textit{Utente}. Presenta un attributo \textit{note} per eventuali informazioni aggiuntive.
...
\vspace{0.5cm}
\end{center}
La sottoclasse \textit{Staff}, analogamente a \textit{Paziente}, è una specializzazione di \textit{Utente}. Ha due attributi aggiuntivi: \textit{Identificativo} per specificarne la funzione e \textit{Stipendio} per specificare il compenso dell'operatore.
...
\vspace{0.5cm}
\end{center}
\textit{Reparto} caratterizza un particolare reparto dell'ospedale (cardiologia, pneumologia, ecc) attraverso l'attributo \textit{Nome}. Inoltre ogni reparto è caratterizzato anche da un \textit{ID} unico. 
...
\vspace{0.5cm}
\end{center}
L'entità \textit{Sala} rappresenta le varie sale disponibili nell'ospedale. Ha come chiave esterna l'\textit{ID} del reparto alla quale è assegnata. 
...
\vspace{0.5cm}
\end{center}
L'entità \textit{Referto} è una entità debole, collegata a prestazione. \textit{Referto} è stato introdotto come entità debole in quanto non può esistere senza una corrispondente prestazione.
...
\begin{tikzpicture}[
  ...
  %	%shorten >=1pt,
	...
	\node[entity] (permesso) {Permesso};
	
	\node[attribute] (id) [right of = permesso] {\key{ID}} edge (permesso);
	\node[attribute] (nome) [left of = permesso] {Nome} edge (permesso);
	\node[attribute] (descrizione) [above right of = permesso] {Descrizione} edge (permesso);
	\node[attribute] (timestamp) [above left = 1cm of permesso] {Timestamp} edge (permesso);				
\end{tikzpicture}
\vspace{0.5cm}
\end{center}
L'entità \textit{Permesso} identifica i permessi sull'applicazione concessi ad un determinato utente.
