...
\begin{itemize}
\item La sottoclasse \textit{Paziente} che presenta ulteriori attributi quali: \textit{altezza}, \textit{peso} e \textit{note} per eventuali informazioni aggiuntive;
...
\begin{tikzpicture}[
  ...
  %	%shorten >=1pt,
	...
	\node[entity] (permesso) [left = 1cm of detiene] {Permesso} edge node{N} (detiene);
	\node[entity] (utente) {Utente} edge [total] node {M} (detiene);
	\node[isa] (isa) [below of = utente] {d} edge [total] (utente);	
